...
if (root) {
  ...
  async function api(url, options = {}) { const metodo = String(options.method || 'GET').toUpperCase(); if (url === '/api/publicaciones' && metodo === 'GET') { const response = await fetch('/datos/publicaciones.json'); if (!response.ok) throw new Error('request'); return await response.json(); } throw Object.assign(new Error('soloLectura'), { code: 'soloLectura' }); }
